...
- [ ] T001 Delta inventory: `git diff --name-status v0.1.1..HEAD`,
...
- [ ] T002 [P] Bring up the SFTP reference server: start Docker Desktop,
...
- [ ] T003 [P] Start gate G1 `build.cmd full` (Debug, background) — also
...
- [ ] T004 [US1] Author and run the review Workflow per
...
- [ ] T007 [US2] Gate G3: build the `saltests` project (Debug x64) and run
      the exe; exit code 0 (unit tests of `src/common/salunicode.cpp` /
      `salpath.cpp` — the 052 surface)
- [ ] T008 [US2] Gate G4 (needs T002, T003):
      `src\plugins\sftp\test\run_keyauth.cmd` — all 7 scenarios pass
      including the CRT leak check
- [ ] T009 [P] [US2] Gate G5: `src\plugins\sftp\test\build_and_run.cmd`
      key-format fixtures pass
- [ ] T010 [P] [US2] Gate G6: `python -m translate.slt --verify`
      (PYTHONPATH=tools) byte-exact on all committed `.slt`
- [ ] T011 [US2] Gate G7 (needs T003): smoke — set Czech UI language in
...
- [ ] T012 [US2] Gate G2: `build.cmd full release` (background) — 0 errors
- [ ] T013 [US2] Consolidate the gate table in review-report.md: G1–G8 all
...
- [ ] T014 [US3] Stamp (needs T013): `src/plugins/shared/spl_vers.h` —
...
- [ ] T017 [US3] Gate G9 sweep: grep the four mandated files for the new
...
- [ ] T018 Finalize `specs/056-prerelease-review/review-report.md`:
