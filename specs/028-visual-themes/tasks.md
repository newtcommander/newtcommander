...
- [ ] T001 Create `src/themes.h` — API per contracts/theme-engine.md
...
- [ ] T002 Create `src/themes.cpp` — dark chrome palette table
...
- [ ] T003 Add `DarkColors[NUMBER_OF_COLORS]` + `DarkViewerColors[4]`
...
- [ ] T004 Add `DWORD ThemeMode` to `CConfiguration` in `src/cfgdlg.h`
...
- [ ] T005 Early theme read: load `CONFIG_THEMEMODE_REG` next to the
      `CONFIG_SHOWSPLASHSCREEN_REG` quick read in `src/salamdr1.cpp`
      (~3958), before `SplashScreenOpen`
- [ ] T006 Decouple scheme pointer: introduce `SchemeColors`; convert the
...
- [ ] T007 Viewer palette indirection: add `CurrentViewerColors` pointer;
...
- [ ] T008 Register `themes.cpp`/`themes.h` in
      `src/vcxproj/salamand.vcxproj` (+ filters file if present)
- [ ] T009 Build gate: `build.cmd` (Debug x64) compiles clean; app starts;
...
- [ ] T010 [US1] Add `CM_THEME_DEFAULT`/`CM_THEME_DARK` to
...
- [ ] T011 [US1] Add "Theme" submenu (MNTT_PB + 2 MNTT_IT rows, all skill
      levels) to Options popup in `src/menu4.cpp` (~180-256)
- [ ] T012 [US1] Implement `SetThemeMode` full switch sequence
...
- [ ] T013 [US1] WM_COMMAND dispatch: `case CM_THEME_DEFAULT/DARK` beside
...
- [ ] T014 [US2] Startup application: after LoadConfig
...
- [ ] T015 [US1] Build + verify: switch recolors panels, panel captions,
...
- [ ] T016 [US1] Convert cached GDI: six `GetSysColorBrush` globals →
...
- [ ] T022 [P] [US1] Tooltip: `src/tooltip.cpp` (644 INFOBK fill, 646
      INFOTEXT) → accessors
- [ ] T023 [US1] Graphics baking: rebar bg (`src/salamdr1.cpp:2997`),
...
- [ ] T024 [US1] SVG icons: route `GetSVGSysColor` (`src/svg.cpp:35`)
      through ThemeSysColor so toolbar/menu sprites rasterize for dark
- [ ] T025 [US1] Main-window non-dialog surfaces: `src/mainwnd3.cpp`
      pens/brushes (6740-6803) → accessors; verify tab/drive bars pick
      up imagery from T023
- [ ] T026 [US1] Build + visual check of main window in Dark (screenshot
...
- [ ] T027 [US3] Implement `ThemeHandleCtlColor` +
...
- [ ] T028 [US3] Hook central procs: `CDialog::DialogProc`
...
- [ ] T030 [P] [US3] Reconcile precedents: `src/msgbox.cpp`
      ...
      verified compatible
- [ ] T031 [P] [US3] Find window: `src/finddlg1.cpp` results custom draw
      (4003-4079, 4460) + `src/finddlg2.cpp:559` → accessors (keep the
      high-contrast test on raw GetSysColor); status bar dark
- [ ] T032 [P] [US3] Remaining dialog draw sites: `src/dialogs3.cpp`
...
- [ ] T033 [US3] Viewer + Find DWM title bars on creation
...
- [ ] T034 [US3] Build + dialog walkthrough in Dark (config all pages,
...
- [ ] T035 [US4] Owner-drawn buttons/checkboxes dark branch in
...
- [ ] T039 [P] saltests theme suite (new test file in saltests project):
...
- [ ] T040 Run full saltests — all pre-existing 427 + new must pass
- [ ] T041 `build.cmd` Debug x64 AND `build.cmd full release` Release
      x64 compile clean; smoke-launch built exe (start + exit)
- [ ] T042 clang-format all changed files; UTF-8-BOM check on new files
- [ ] T043 Update `specs/028-visual-themes/quickstart.md` walkthrough
      with any deviations; hand off GUI walkthrough (SC-001..SC-007,
      incl. 20-switch stability + high-contrast check) to user
- [ ] T044 Re-verify FR-005/SC-003: diff-review that every converted
