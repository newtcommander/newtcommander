...
* ``translations/languages.cfg`` -- which languages ship, and the metadata that
  lands in each ``.slg``'s VERSIONINFO.
...
def load_languages(path: Path | None = None) -> list[Language]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"{len(languages)} languages x {len(modules)} modules "
          f"= {len(languages) * len(modules)} language modules "
          f"(+{len(modules)} english)")
    print()
    print(f"{'language':<20} {'langid':>6}  {'origin':<8} {'with source':>11}")
    print("-" * 50)
    for lang in languages:
        have = sum(1 for m in modules if has_translation_source(m, lang))
        print(f"{lang.folder:<20} {lang.langid:>6}  {lang.origin:<8} {have:>7}/{len(modules)}")
    print()
    missing = [
        m.name
        for m in modules
        if not any(has_translation_source(m, l) for l in languages)
    ]
    if missing:
