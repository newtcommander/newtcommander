...
if not exist "%OPENSAL_BUILD_DIR%tandemcommander" (
  echo Build directory does not exist: %OPENSAL_BUILD_DIR%tandemcommander
  echo.
  pause
  exit /b
)
...
echo.
echo Press any key to copy SFX packages to %OPENSAL_BUILD_DIR%tandemcommander...
echo.
pause
...
for %%t in (Debug_x86 Release_x86 Debug_x64 Release_x64) do (
  ...
  if not exist "%OPENSAL_BUILD_DIR%tandemcommander\%%t\plugins\zip\sfx" mkdir "%OPENSAL_BUILD_DIR%tandemcommander\%%t\plugins\zip\sfx"
  for %%s in (czech.sfx english.sfx german.sfx slovak.sfx spanish.sfx ^
    ...
    @call :sfxcopy "sfx\%%s" "%OPENSAL_BUILD_DIR%tandemcommander\%%t\plugins\zip\sfx\%%s"
  )
)
